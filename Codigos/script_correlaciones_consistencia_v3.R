...
# Se almacenan los distintos df en una lista
# calculo alpha muestra completa
for (i in 1: length(lista_conceptos_especificos)) {
...
# muestra 1
el.1 <- el %>% filter(muestra == 1) %>% 
  select(where(~!all(is.na(.)))) # eliminar columnas donde todos los valores son NA

# calculo alphas muestra 1
for (i in 1: length(lista_conceptos_especificos)) {
  v <- unlist(lista_conceptos_especificos[i])
  df <- el.1 %>% dplyr::select(starts_with(v)) 
  list_df[[i]] <- df
  names(list_df)[i] <- names(lista_conceptos_especificos)[i]
  for (j in 1:6) {
    # Se separan las variables según sus respectivas olas
    ...
    if (length(df_subset) > 2) {
      ...
      alfa_df$total_alfa <- alfa_v$total$raw_alpha
      name_alpha <- paste("Alpha", i,j, sep = "_")
      alfas.1[[name_alpha]] <- alfa_df
    } else if(length(df_subset)==2) {
      correlation <- cor(df_subset,use="pairwise.complete.obs")
      name_cor <- paste("Correlacion", i,j, sep = "_")
      alfas.1[[name_cor]] <- correlation
    } else if(length(df_subset)==1) {
      alfas.1[[i]] <- "No aplica"
      name_no.aplica <- l
      alfas.1[[name_no.aplica]] <- alfas.1[[i]]
    }
  }
}

# Se seleccionan todos los df del loop anterior
alfas_df_sub.1 <- alfas.1[sapply(alfas.1,is.data.frame)]

# muestra 2
el.2 <- el %>% filter(muestra == 2) %>% 
  select(where(~!all(is.na(.)))) # eliminar columnas donde todos los valores son NA

# calculo alphas muestra 2
for (i in 1: length(lista_conceptos_especificos)) {
  v <- unlist(lista_conceptos_especificos[i])
  df <- el.2 %>% dplyr::select(starts_with(v)) 
  list_df[[i]] <- df
  names(list_df)[i] <- names(lista_conceptos_especificos)[i]
  for (j in 1:6) {
    # Se separan las variables según sus respectivas olas
    ...
    if (length(df_subset) > 2) {
      ...
      alfa_df$total_alfa <- alfa_v$total$raw_alpha
      name_alpha <- paste("Alpha", i,j, sep = "_")
      alfas.2[[name_alpha]] <- alfa_df
    } else if(length(df_subset)==2) {
      correlation <- cor(df_subset,use="pairwise.complete.obs")
      name_cor <- paste("Correlacion", i,j, sep = "_")
      alfas.2[[name_cor]] <- correlation
    } else if(length(df_subset)==1) {
      alfas.2[[i]] <- "No aplica"
      name_no.aplica <- l
      alfas.2[[name_no.aplica]] <- alfas.2[[i]]
    }
  }
}

# Se seleccionan todos los df del loop anterior
alfas_df_sub.2 <- alfas.2[sapply(alfas.2,is.data.frame)]
